...
A .net and springboot implementation of the api is in place.
...
How to create an image you can refer to the [Dockerfile](./api/Dockerfile).

### .net
The api is implemented via .NET [here](./dotnet-api) and can be launched by. 
```console
dotnet run
```
The api listens on port 5242 but this can be changed by configuration setting easily.

The PostgreSql database is accessed by entity framework and the corresponding context is [here](./dotnet-api/Repository/BookingDbContext.cs).
For running as container you can use this [Dockerfile](./dotnet-api/Dockerfile).
